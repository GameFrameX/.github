...
一個人做連網遊戲，要扛的遠不止玩法：選客戶端引擎、寫伺服器、選資料庫、搭配置表流程、配發布 CI——上線之後，還得有個後台看玩家數據。單拎出來沒有一件難；難的是每一件都落在你一個人頭上。

GameFrameX 一次性把這些都給你：多引擎客戶端、多進程伺服器、雙資料庫、配置表工具、網路協定、一體化後台、三平台 CI。你只管專注玩法，剩下的，框架已經組裝好了。

遊戲引擎延伸了你的雙手；GameFrameX 延伸的是你的整個團隊——伺服器是你維運能力的延伸，後台是你營運能力的延伸，CI 是你發布節奏的延伸。一個人加上 GameFrameX，就擁有過去一個團隊的火力。

獨立開發不該是孤軍奮戰：框架是武器，文檔是地圖，社群是同行的族人。從第一個原型到千萬玩家，每一步都有同類同行。
...
1. 打開[快速開始指南](https://gameframex.doc.alianblank.com)
2. 按步驟把 GameFrameX 接入你的專案
3. 推第一個 `tag`，看 CI 把它變成一個可運行的服

從這一刻起，你面對的不是一堆零件，而是一條已經接好的流水線。
